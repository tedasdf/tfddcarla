...
class FollowLeadingVehicleMultiActor(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._map = CarlaDataProvider.get_map()
        self._first_vehicle_location = 25
        self._first_vehicle_speed = 1000
        self._second_vehicle_location = 70
        ...
        super(FollowLeadingVehicleMultiActor, self).__init__("FollowLeadingVehicleMultiActor",
                                                   ego_vehicles,
                                                   config,
                                                   world,
                                                   debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        Custom initialization
        """
        waypoint1, _ = get_waypoint_in_distance(self._reference_waypoint, self._first_vehicle_location)
        transform1 = waypoint1.transform
        transform1.location.z += 0.5

        waypoint2, _ = get_waypoint_in_distance(self._reference_waypoint, self._second_vehicle_location)
        transform2 = waypoint2.transform
        transform2.location.y -= 3.5
        transform2.location.z += 0.5
        transform2.rotation.yaw += -180
        
        # if we specify a non-default model in the config
        if config.other_actors: 
            first_vehicle = CarlaDataProvider.request_new_actor(config.other_actors[0].model, transform1)
        else:
            first_vehicle = CarlaDataProvider.request_new_actor('vehicle.tesla.model3', transform1)

        second_vehicle = CarlaDataProvider.request_new_actor(config.other_actors[0].model, transform2)

        # spawns walkers clankers
        # walker_tf = waypoint1.transform
        # walker_tf.location.y += 1
        # walker = CarlaDataProvider.request_new_actor('controller.ai.walker', walker_tf)
    ...
    def _create_behavior(self):
        ...
        driving_to_next_intersection.add_child(WaypointFollower(self.other_actors[1], self._second_vehicle_speed))
