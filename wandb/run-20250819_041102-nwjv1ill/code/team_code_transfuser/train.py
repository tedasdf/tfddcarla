...
def main():
    print("type shit")
    wandb = wdb.init(project="tfuse", name="tfddcarla")
    ...
    if (not (args.load_file is None)):
        ...
        model.load_state_dict(torch.load(
            args.load_file, map_location=model.device))
        optimizer.load_state_dict(torch.load(args.load_file.replace(
            "model_", "optimizer_"), map_location=model.device))
...
class Engine(object):
    ...
    def train(self):
        ...
        self.log_losses(loss_epoch, detailed_losses_epoch, num_batches, pred_wp, '')
    ...
    def validate(self):
        ...
        self.log_losses(loss_epoch, detailed_val_losses_epoch,
                        num_batches, pred_wp, 'val_')

    def log_losses(self, loss_epoch, detailed_losses_epoch, num_batches, pred_wp, prefix=''):
        ...
        self.wandb.log({"epoch": self.cur_epoch, "gathered_loss": gathered_loss, "gathered_detailed_losses": gathered_detailed_losses, 'pred_wp':pred_wp})
    ...
    def save(self):
        ...
        torch.save(self.model.state_dict(), os.path.join(
            self.args.logdir, 'model_%d.pth' % self.cur_epoch))
        torch.save(self.optimizer.state_dict(), os.path.join(
            self.args.logdir, 'optimizer_%d.pth' % self.cur_epoch))
